...
@SeqName_list = (
    ...
    BasketballPass, ChinaSpeed
);

open( my $logfile, ">", "HEVC_vs_EI.csv");
print $logfile "Sequence, HEVC_PSNR, HEVC_Bits, HEVC_Time, EI_PSNR, EI_Bits, EI_Time, Ratio_Bits, x_Time\n";
$avg_ratio = 0;
$avg_x_time = 0;
foreach $SeqName_name (@SeqName_list) {
    ...
    foreach $line_HEVC (@lines_HEVC) {
        if ( $line_HEVC =~ /(?<=\s{3})\d+\.\d{4}(?!\s{3}\d)/ ) {
            $PSNR_HEVC = $&;
            # print $PSNR_HEVC;
            # print "    ";
        }
        elsif ( $line_HEVC =~ /(?<=Bytes\ written\ to\ file:)\s*\d+/ ) {
            $Bits_HEVC = $&;
            # print $Bits_HEVC;
            # print "    ";
        }
    ...
    foreach $line_EI (@lines_EI) {
        if ( $line_EI =~ /(?<=\s{3})\d+\.\d{4}(?!\s{3}\d)/ ) {
            $PSNR_EI = $&;
            # print $PSNR_EI;
            # print "    ";
        }
        elsif ( $line_EI =~ /(?<=Bytes\ written\ to\ file:)\s*\d+/ ) {
            $Bits_EI = $&;
        }
